...
interface BirthdayDao {
    ...
    @Query("SELECT * FROM BirthdayEntity WHERE idGroup = :idGroup")
    suspend fun getAllByGroup(idGroup: Int): List<BirthdayEntity>

    @Query("SELECT * FROM BirthdayEntity WHERE idGroup = :idGroup")
    suspend fun getByGroup(idGroup: Int): BirthdayEntity

    @Query("SELECT * FROM BirthdayEntity WHERE name = :name")
    suspend fun getAllByName(name: String): List<BirthdayEntity>

    @Query("SELECT * FROM BirthdayEntity WHERE name = :name")
    suspend fun getByName(name: String): BirthdayEntity

    @Query("SELECT * FROM birthdayentity WHERE name = :name and idGroup = :idGroup")
    suspend fun getAllByNameAndGroup(name: String, idGroup: Int): List<BirthdayEntity>

    @Query("SELECT * FROM birthdayentity WHERE name = :name and idGroup = :idGroup")
    suspend fun getByNameAndGroup(name: String, idGroup: Int): BirthdayEntity
